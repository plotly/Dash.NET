...
let ``Component prop types serialization tests`` =
    testList "SerializationTests.ComponentPropTypes" [
        testCase "LoadingState" (fun _ ->
            let subject = LoadingState.init(true, "Hi", "yes.") |> json
            Expect.equal 
                subject 
        ...
        testCase "DropdownOption" (fun _ ->
            let subject = DropdownOption.init("Label", 42, false, "yes.") |> json
            Expect.equal 
                subject 
        ...
        testCase "RadioItemsOption" (fun _ ->
            let subject = RadioItemsOption.init("Label", 42, false) |> json
            Expect.equal 
                subject 
        ...
        testCase "TabColors" (fun _ ->
            let subject = TabColors.init("#ffffff", "#000000", "violet") |> json
            Expect.equal 
                subject 
        ...
        testCase "ChecklistOption" (fun _ ->
            let subject = ChecklistOption.init("Label", 42, false) |> json
            Expect.equal 
                subject 
                """{"label":"Label","value":42,"disabled":false}"""
